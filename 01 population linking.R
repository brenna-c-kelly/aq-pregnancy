...
# summary file
v_10 <- load_variables(2010, "sf1", cache = TRUE)
...
nrow(pop_xwalk) / nrow(block_xwalk_ut) # excludes 62% of the grids
...
# Martin > then run this <3
no2_points <- st_drop_geometry(no2_dat)
no2_points <- st_as_sf(no2_points, coords = c("long.x", "lat.x"), # coodinates of centroid
                       crs = projcrs, agr = "constant")
...
pts_in_gc <- st_contains(no2_points, gc_crop)
as.matrix(pts_in_gc)

cbind(pts_in_gc, no2_points)
